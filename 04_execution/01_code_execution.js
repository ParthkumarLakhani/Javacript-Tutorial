...
/* Full architecture diagram: [js]
┌──────────────────────────────────────────────────────────┐
│                        JS ENGINE                         │
│                                                          │
│  ┌──────────────┐    ┌────────────────────────────┐      │
│  │ CALL STACK   │    │           HEAP             │      │
│  │ (execution)  │    │ (data: objects/functions)  │      │
│  └──────────────┘    └────────────────────────────┘      │
│                                                          │
└──────────────────────────────────────────────────────────┘
                      |
...
/* Node.js Internal Architecture — Visual Representation

Node.js = V8 (JavaScript Engine) + libuv (Async I/O Engine) + C++ APIs + Node Internals

🧠 1. V8 Memory Model (Same as Browser)
           ┌─────────────────────────────┐
           │           HEAP               │
           │  (Objects, Arrays,           │
           │   Functions, Closures)       │
...
✔ V8 handles stack + heap + execution context
✔ Node adds extra layers below this (libuv, OS threads, etc.)
...
│  ┌───────────────┐    ┌─────────────────────────────────┐   │
│  │   V8 Engine    │    │           libuv                 │   │
│  │  (JS runtime)  │    │  (Async I/O engine)             │   │
│  └───────────────┘    └─────────────────────────────────┘   │
│                                                             │
│    Modules Layer (fs, crypto, http, net, dns, timers, etc)  │
│                                                             │
└─────────────────────────────────────────────────────────────┘


🔥 3. Detailed Node.js Internal Flow (Best Visual)

          ┌─────────────────────────────┐
          │           V8                │
          │  CALL STACK + HEAP          │
          └─────────────────────────────┘
                     │ JS calls native APIs
                     ▼
       ┌───────────────────────────────────────┐
       │               Node APIs                │
       │  fs, crypto, net, http, timers, dns    │
       └───────────────────────────────────────┘
                     │ Calls into libuv
                     ▼
       ┌───────────────────────────────────────┐
     ...
     ┌───────────────┬────────────────────────────────────────┐
     ▼               ▼                                          ▼
Timers Phase   Pending Callbacks Phase                 I/O Poll Phase
     ▼               ▼                                          ▼
Check Phase      Close Callbacks Phase      Microtasks handled after each phase



🧵 4. Node.js Event Loop — 6 Phases (Very Important)
...
│     - setInterval                                           │
├─────────────────────────────────────────────────────────────┤
│ 2. **Pending Callbacks** (OS callbacks)                      │
│     - TCP errors, socket callbacks                           │
├─────────────────────────────────────────────────────────────┤
│ 3. **Idle / Prepare** (internal)                             │
├─────────────────────────────────────────────────────────────┤
│ 4. **Poll Phase**                                            │
│     - Main I/O happens here (fs, network, database, etc.)    │
...
🧠 5. Microtask Queue (Runs After Every Phase!)
...
Order of execution in Node:
  1. process.nextTick()
  2. Promise microtasks
  3. Move to next phase of Event Loop



💪 6. libuv Thread Pool (VERY IMPORTANT in Node.js)

Node.js is single-threaded, but libuv uses a Thread Pool (4 threads by default) for heavy tasks:
      ┌─────────────────────────┐
      │  libuv THREAD POOL      │
      │  (parallel workers)     │
      ├─────────────────────────┤
...
So Node.js can handle heavy work without blocking the main thread.




⚡ 7. Final Full Visual (The MOST COMPLETE Diagram)
                 ...
                 ┌──────────────┬───────────────┬───────────────┐
                 ▼              ▼               ▼                ▼
        Timers Phase   Pending Callbacks   Poll Phase    Check Phase
                 ▼              ▼               ▼                ▼
